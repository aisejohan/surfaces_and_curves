...
struct polynomial **all_the_way_split(struct polynomial **bb)
{
	...
	for(ii=0;ii<=jj-1;ii++) {
		...
		cc = all_the_way(bb[ii]);
		rep_pol_add(aa[0],*cc[0]);
		free_tail(cc[0]->leading);
		free(cc[0]);
		rep_pol_add(aa[1],*cc[1]);
		free_tail(cc[1]->leading);
		free(cc[1]);
		rep_pol_add(aa[2],*cc[2]);
		free_tail(cc[2]->leading);
		free(cc[2]);
		free(cc);
		 ...
		 * don't make an error.					*
		 * Note that k is cumulative (not reset back to 1)	*/
		if(j>1) {
			i = j-1;
			while(i % p == 0) {
				i = i/p;
