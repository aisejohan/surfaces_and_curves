	...
	int glen3, struct term **gens3)
{
	...
	matrix = (mscalar **)malloc((blen1 + blen2 + blen3)*sizeof(mscalar *));
	if (!matrix) {
		perror("Malloc failed!\n");
		exit(1);
	}
	for (i = 0; i < blen1 + blen2 + blen3; i++) {
		matrix[i] = (mscalar *)
			malloc((glen1 + glen2 + glen3)*sizeof(mscalar));
		if (!matrix[i]) {
			perror("Malloc failed!\n");
			exit(1);
		}
		for (j = 0; j < glen1 + glen2 + glen3; i++) {
			make_scalar(matrix[i][j]);
			sc_zero(matrix[i][j]);
		}
	}
