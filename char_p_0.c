...
void print_terms(struct term **tt, int blen, int degree)
{
	int i;
	struct polynomial T;
	...
	int blen3, struct term **basis3)
{
	...
	while ((aaterm) && (row < blen3)) {
		...
		(aaterm->n3 == basis3[row]->n3)) {
			sc_copy((mscalar) aaterm, column[row]);
			row++;
			aaterm = aaterm->next;
		} else {
			row++;
...
struct polynomial **split_up_term(struct term *tt, int degree)
{
	...
	if (sc_is_zero((mscalar) tt)) return(aa);
	...
	int *e)
{
	...
	while (i < glen1 + glen2 + glen3) {
		if (i < glen3) {
			...
			sc_copy(c, (mscalar) gens1[i - glen3 - glen2]);
